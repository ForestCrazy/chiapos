...
public:
    ...
    explicit DiskProver(const std::string& filename) : id(kIdLen)
    {
        ...
        if (magic_2_result == CHIA_PLOT_V2_MAGIC) {
            ...
            readData(tmp_magic_buff, sizeof(header.magic) - 4);
        ...
        readData((uint8_t*)&header.id, sizeof(header.id));
        readData((uint8_t*)&header.k, sizeof(header.k));

        if (version == 1) {
            readData((uint8_t*)&header.fmt_desc_len, sizeof(header.fmt_desc_len));
            readData((uint8_t*)&header.fmt_desc, sizeof(header.fmt_desc));
        ...
        if (version == 2) {
            ...
            readData(flags_bytes, sizeof(flags_bytes));
            uint32_t flags;
            memcpy(&flags, flags_bytes, sizeof(flags));
            if (flags & 1) {
                uint8_t compression_level;
                readData(&compression_level, sizeof(compression_level));
